...
default_selection[["Orange Lake"]] = list(strata = FALSE, 
                                             primary = 25, 
                                             alternate = 5,
                                             map_lims_x = c(-82.21494, -82.10933),
                                             map_lims_y = c(29.41866, 29.51579))
default_selection[["Lochloosa Lake"]] = list(strata = FALSE, 
                                          primary = 25, 
                                          alternate = 2,
                                          map_lims_x = c(-82.1645, -82.10300),
                                          map_lims_y = c(29.48500, 29.56900)
                                         )
default_selection[["Newnans Lake"]] = list(strata = FALSE, 
                                             primary = 25, 
                                             alternate = 1,
                                             map_lims_x = NA,
                                             map_lims_y = NA,
...
format_siteList <- function(data, column_mapping = list(), column_fill = list()) {
  temp = data
...
random.sites <- function(waterbody = NA, selection_scheme = default_selection, sitelist=NA) {
  ...
  if(ss[["strata"]]==FALSE) {
    ...
    for(x in 1:n_strata) {
      ...
      site_sub = site_list %>% filter(Zone == cur_zone)
  ...
  if(sum(!is.na(sites$Zone)) == 0) {sites$Zone = as.factor(1)}
  sites = sites %>% 
    arrange(Zone, ident, desc(selection)) %>% 
...
output_gpx <- function(x, filename) {
  pgirmess::writeGPX(data.frame(x), filename=filename, type="w")
...
import.rivermap <- function(shapefile, filter) {
fc <- raster::shapefile(shapefile)
fc <- ggspatial::df_spatial(fc) %>% dplyr::filter(NAME == filter)
fc
}
...
plot.sitemap <- function(basemap, 
  ...
  if(!is.null(default_selection[[wb]])) {
    if(!is.na(default_selection[[wb]]$map_lims_x)){
      map_xlim = default_selection[[wb]]$map_lims_x}
    if(!is.na(default_selection[[wb]]$map_lims_y)){
