  ...
  if(ss[["strata"]]==FALSE) {
    ...
    sites = sites_primary %>% bind_rows(sites_alternate) %>% arrange(ident)
      ...
      names(pick_strata) == strata} else {
        if(length(ss$primary) == n_strata) {
          pick_strata = ss$primary
        } else {
            stop(paste("strata selection vector length does 
                    not match number of strata::strata are: ",strata))
          }
      }
      
      
      if(length(ss$alternate) == 1) {
        pick_alt = rep(ss$alternate,n_strata)
        names(pick_alt) == strata
        } else if(length(ss$alternate) == n_strata) {
        pick_alt = ss$alternate
        } else {
        stop("alt strata selection vector length does 
